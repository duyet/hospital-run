...
if [ -z "${1}" ] || [ -z "${2}" ]; then
    ...
    curl -X PUT $HOST/_config/admins/duyetdev -d '"123456"'
...
curl -X PUT $SECUREHOST/_users/org.couchdb.user:admin -d '{"name": "admin", "password": "admin", "roles": ["System Administrator","admin","user"], "type": "user", "userPrefix": "p1"}'
